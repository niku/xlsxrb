...
For behavioral policies, constraints, and component-level quality gate requirements governing AI coding agents, see [AGENTS.md](../AGENTS.md).
...
10. Verification Suite: Run the pre-push verification suite (`bin/pre-push`). Ensure your changes satisfy the component quality gates defined in [AGENTS.md](../AGENTS.md).
11. Changelog & Commit: Update `CHANGELOG.md` under `## [Unreleased]` for any user-facing changes (features, deprecations, breaking changes, notable bug fixes, performance improvements, or dependency changes) following [Keep a Changelog](https://keepachangelog.com/) conventions. Commit the changes. The commit message must clearly describe the specific feature implemented in this cycle.
