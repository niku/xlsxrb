// Validates gapDepth and shape elements for a 3D bar chart.
...
try
{
    ...
    if (!chartXml.Contains("gapDepth"))
        throw new Exception("SCENARIO_FAIL: gapDepth not found in chart XML");
    if (!chartXml.Contains("<c:shape"))
        throw new Exception("SCENARIO_FAIL: shape not found in chart XML");
