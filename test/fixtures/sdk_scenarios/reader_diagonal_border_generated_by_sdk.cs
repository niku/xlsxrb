...
var borders = new Borders(
    ...
    new Border(
        ...
        new TopBorder(),
        new BottomBorder(),
        new DiagonalBorder(new Color { Rgb = "FFFF0000" }) { Style = BorderStyleValues.Thin }
...
var cellXfs = new CellFormats(
    ...
    new CellFormat { NumberFormatId = 0, FontId = 0, FillId = 0, BorderId = 1, FormatId = 0, ApplyBorder = true }
);
cellXfs.Count = 2;
...
row.Append(new Cell { CellReference = "A1", DataType = CellValues.InlineString, StyleIndex = 1U,
    InlineString = new InlineString(new Text("diagonal")) });
sd.Append(row);
ws.Append(sd);
wsPart.Worksheet = ws;
