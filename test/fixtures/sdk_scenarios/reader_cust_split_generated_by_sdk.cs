// Generates an XLSX with an ofPieChart having custSplit with secondPiePt elements.
...
try
{
    ...
    wsPart.Worksheet = new Worksheet(new SheetData(
        new Row(new Cell { CellReference = "A1", DataType = CellValues.Number, CellValue = new CellValue("10") },
                ...
                new Cell { CellReference = "C1", DataType = CellValues.Number, CellValue = new CellValue("30") },
                new Cell { CellReference = "D1", DataType = CellValues.Number, CellValue = new CellValue("40") })
    ));
  ...
  <c:plotArea>
    ...
    <c:ofPieChart>
      ...
      <c:ser>
        <c:idx val=""0""/>
        <c:order val=""0""/>
        <c:val><c:numRef><c:f>Sheet1!$A$1:$D$1</c:f></c:numRef></c:val>
      </c:ser>
