// Validates that an XLSX shape pPr has marL, marR, and indent attributes.
...
try
{
    ...
    var marL = pPr.LeftMargin ?? throw new Exception("LeftMargin is missing.");
    if (marL.Value != 457200)
        throw new Exception($"Expected marL=457200, got {marL.Value}.");

    var marR = pPr.RightMargin ?? throw new Exception("RightMargin is missing.");
    if (marR.Value != 228600)
        throw new Exception($"Expected marR=228600, got {marR.Value}.");

    var indent = pPr.Indent ?? throw new Exception("Indent is missing.");
    if (indent.Value != -114300)
        throw new Exception($"Expected indent=-114300, got {indent.Value}.");
