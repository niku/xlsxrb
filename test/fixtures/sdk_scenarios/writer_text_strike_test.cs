// Validates that an XLSX shape text run has strike attribute on a:rPr.
...
try
{
    ...
    if (rPr.Strike?.Value != DocumentFormat.OpenXml.Drawing.TextStrikeValues.SingleStrike)
        throw new Exception($"Expected strike=sngStrike, got {rPr.Strike?.Value}.");
