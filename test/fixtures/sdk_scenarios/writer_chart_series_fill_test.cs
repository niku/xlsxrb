// Validates fill color on chart series via c:spPr solidFill.
...
try
{
    ...
    if (!chartXml.Contains("solidFill"))
        throw new Exception("SCENARIO_FAIL: solidFill not found in chart XML");
    if (!chartXml.Contains("00FF00"))
        throw new Exception("SCENARIO_FAIL: fill color 00FF00 not found in chart XML");
