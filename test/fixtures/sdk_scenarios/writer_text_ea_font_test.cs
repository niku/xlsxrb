// Validates that an XLSX shape text run has a:ea East Asian font element.
...
try
{
    ...
    var ea = rPr.Elements<DocumentFormat.OpenXml.Drawing.EastAsianFont>().FirstOrDefault()
        ?? throw new Exception("EastAsianFont element is missing.");

    if (ea.Typeface?.Value != "MS Gothic")
        throw new Exception($"Expected ea typeface=MS Gothic, got {ea.Typeface?.Value}.");
