// Validates that an XLSX shape text run has a:cs Complex Script font element.
...
try
{
    ...
    var cs = rPr.Elements<DocumentFormat.OpenXml.Drawing.ComplexScriptFont>().FirstOrDefault()
        ?? throw new Exception("ComplexScriptFont element is missing.");

    if (cs.Typeface?.Value != "Arabic Typesetting")
        throw new Exception($"Expected cs typeface=Arabic Typesetting, got {cs.Typeface?.Value}.");
