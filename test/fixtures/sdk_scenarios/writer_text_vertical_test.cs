// Validates that an XLSX shape bodyPr has vert attribute.
...
try
{
    ...
    if (bodyPr.Vertical?.Value != DocumentFormat.OpenXml.Drawing.TextVerticalValues.Vertical)
        throw new Exception($"Expected vert=vert, got {bodyPr.Vertical?.Value}.");
