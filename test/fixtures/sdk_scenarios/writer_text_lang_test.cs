// Validates that an XLSX shape text run has lang attribute on a:rPr.
...
try
{
    ...
    if (rPr.Language?.Value != "en-US")
        throw new Exception($"Expected lang=en-US, got {rPr.Language?.Value}.");
