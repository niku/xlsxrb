...
try
{
    ...
    var a1 = FindCell("A1");
    if (a1.DataType?.Value != CellValues.Boolean)
    {
        throw new Exception($"A1 should be Boolean but got '{a1.DataType?.Value}'.");
    }
    if (a1.CellValue?.Text != "1")
    {
        throw new Exception($"Expected A1 to be '1' (true) but got '{a1.CellValue?.Text}'.");
    }

    var b1 = FindCell("B1");
    if (b1.DataType?.Value != CellValues.Boolean)
    {
        throw new Exception($"B1 should be Boolean but got '{b1.DataType?.Value}'.");
    }
    if (b1.CellValue?.Text != "0")
    {
        throw new Exception($"Expected B1 to be '0' (false) but got '{b1.CellValue?.Text}'.");
    }
}
finally
{
