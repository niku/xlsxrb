// Validates that an XLSX shape text run has underline attribute on a:rPr.
...
try
{
    ...
    if (rPr.Underline?.Value != DocumentFormat.OpenXml.Drawing.TextUnderlineValues.Single)
        throw new Exception($"Expected u=sng, got {rPr.Underline?.Value}.");
