// Validates that an XLSX shape text run has baseline attribute on a:rPr.
...
try
{
    ...
    if (rPr.Baseline?.Value != 30000)
        throw new Exception($"Expected baseline=30000, got {rPr.Baseline?.Value}.");
