// Validates chart data table (c:dTable) element.
...
try
{
    ...
    if (!chartXml.Contains("dTable"))
        throw new Exception("SCENARIO_FAIL: dTable not found in chart XML");
    if (!chartXml.Contains("showHorzBorder"))
        throw new Exception("SCENARIO_FAIL: showHorzBorder not found in chart XML");
