// Validates that an XLSX shape has a:headEnd and a:tailEnd in a:ln.
...
try
{
    ...
    var headEnd = ln.Elements<DocumentFormat.OpenXml.Drawing.HeadEnd>().FirstOrDefault()
        ?? throw new Exception("HeadEnd is missing.");
    if (headEnd.Type?.Value != DocumentFormat.OpenXml.Drawing.LineEndValues.Triangle)
        throw new Exception($"Expected headEnd type=triangle, got {headEnd.Type?.Value}.");

    var tailEnd = ln.Elements<DocumentFormat.OpenXml.Drawing.TailEnd>().FirstOrDefault()
        ?? throw new Exception("TailEnd is missing.");
    if (tailEnd.Type?.Value != DocumentFormat.OpenXml.Drawing.LineEndValues.Stealth)
        throw new Exception($"Expected tailEnd type=stealth, got {tailEnd.Type?.Value}.");
