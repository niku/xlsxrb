// Validates that a writer-generated XLSX with trendlineLbl spPr and txPr is valid.
...
if (!errors.Any())
{
    ...
    else if (!xml.Contains("<c:spPr>"))
    {
        Console.Error.WriteLine("SCENARIO_FAIL: missing <c:spPr> inside trendlineLbl");
    }
    else if (!xml.Contains("<c:txPr>"))
    {
        Console.Error.WriteLine("SCENARIO_FAIL: missing <c:txPr> inside trendlineLbl");
    }
    else
    {
