// Validates that a writer-generated XLSX with dispUnitsLbl spPr and txPr is valid.
...
if (!errors.Any())
{
    ...
    {
        Console.Error.WriteLine("SCENARIO_FAIL: missing <c:spPr> inside dispUnitsLbl");
    }
    else if (!xml.Contains("<c:txPr>"))
    {
        Console.Error.WriteLine("SCENARIO_FAIL: missing <c:txPr> inside dispUnitsLbl");
    }
    else
    {
