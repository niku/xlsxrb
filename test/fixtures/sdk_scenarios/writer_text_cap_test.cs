// Validates that an XLSX shape text run has cap attribute on a:rPr.
...
try
{
    ...
    if (rPr.Capital?.Value != DocumentFormat.OpenXml.Drawing.TextCapsValues.All)
        throw new Exception($"Expected cap=all, got {rPr.Capital?.Value}.");
