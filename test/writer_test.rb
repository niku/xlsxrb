...
class WriterTest < Test::Unit::TestCase
  ...
  test "add_shape with fill_alpha emits a:alpha inside a:srgbClr" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-fillalpha", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    drawing_xml = read_xml_from_xlsx(xlsx_path, "xl/drawings/drawing1.xml")
    assert_match(%r{<a:solidFill><a:srgbClr val="FF0000"><a:alpha val="50000"/></a:srgbClr></a:solidFill>}, drawing_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "add_shape with autofit hash normal and fontScale emits attributes on a:normAutofit" do
    ...
    drawing_xml = read_xml_from_xlsx(xlsx_path, "xl/drawings/drawing1.xml")
    assert_match(%r{<a:normAutofit fontScale="90000" lnSpcReduction="20000"/>}, drawing_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "add_shape with line_miter_limit emits lim attribute on a:miter" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-miterlim", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    drawing_xml = read_xml_from_xlsx(xlsx_path, "xl/drawings/drawing1.xml")
    assert_match(%r{<a:miter lim="800000"/>}, drawing_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits chartSpace spPr with chart_line_dash" do
    ...
    xlsx_path = File.join(Dir.tmpdir, "chart_space_ld_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{</c:chart>.*<c:spPr>.*<a:ln><a:solidFill><a:srgbClr val="333333"/></a:solidFill><a:prstDash val="lgDash"/></a:ln>.*</c:spPr>.*</c:chartSpace>}m, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits chartSpace spPr with chart_no_fill" do
    ...
    writer.add_chart(type: :bar,
                     series: [{ val_ref: "Sheet1!$A$1" }],
                     chart_no_fill: true)
    xlsx_path = File.join(Dir.tmpdir, "chart_space_nf_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{</c:chart>.*<c:spPr><a:noFill/></c:spPr>.*</c:chartSpace>}m, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits series marker with fill and line" do
    ...
    xlsx_path = File.join(Dir.tmpdir, "ser_mkfill_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:spPr><a:solidFill><a:srgbClr val="FF0000"/></a:solidFill><a:ln w="12700"><a:solidFill><a:srgbClr val="000000"/></a:solidFill></a:ln></c:spPr></c:marker>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits series marker noFill" do
    ...
    writer.add_chart(type: :line,
                     series: [{ val_ref: "Sheet1!$A$1", marker_symbol: "circle",
                                marker_no_fill: true }])
    xlsx_path = File.join(Dir.tmpdir, "ser_mknf_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:spPr><a:noFill/></c:spPr></c:marker>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits series marker no line" do
    ...
    xlsx_path = File.join(Dir.tmpdir, "ser_mknl_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<a:solidFill><a:srgbClr val="FF0000"/></a:solidFill><a:ln><a:noFill/></a:ln></c:spPr></c:marker>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits axis font properties in txPr" do
    ...
    xlsx_path = File.join(Dir.tmpdir, "axfont_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:catAx>.*<c:txPr>.*<a:defRPr sz="1200" b="1"><a:solidFill><a:srgbClr val="FF0000"/></a:solidFill><a:latin typeface="Arial"/></a:defRPr>.*</c:txPr>.*</c:catAx>}m, xml)
    assert_match(%r{<c:valAx>.*<c:txPr>.*<a:defRPr sz="1000" i="1"/>.*</c:txPr>.*</c:valAx>}m, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits dropLines spPr with line_color, line_width, and line_dash" do
    ...
    xlsx_path = File.join(Dir.tmpdir, "drop_sppr_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:dropLines><c:spPr>.*<a:ln w="12700">.*<a:solidFill>.*<a:srgbClr val="0000FF"/>.*</a:solidFill>.*<a:prstDash val="sysDot"/>.*</a:ln>.*</c:spPr></c:dropLines>}m, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits dPt with line_color and line_width" do
    ...
    xlsx_path = File.join(Dir.tmpdir, "dpt_ln_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:dPt><c:idx val="0"/><c:spPr><a:solidFill><a:srgbClr val="FF0000"/></a:solidFill><a:ln w="25400"><a:solidFill><a:srgbClr val="000000"/></a:solidFill></a:ln></c:spPr></c:dPt>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits dPt with no_line" do
    ...
    xlsx_path = File.join(Dir.tmpdir, "dpt_nln_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:dPt><c:idx val="0"/><c:spPr><a:solidFill><a:srgbClr val="FF0000"/></a:solidFill><a:ln><a:noFill/></a:ln></c:spPr></c:dPt>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits dPt with invertIfNegative" do
    ...
    xlsx_path = File.join(Dir.tmpdir, "dpt_iin_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:dPt><c:idx val="1"/><c:invertIfNegative val="1"/>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits dLbl with delete flag" do
    ...
    writer.add_chart(type: :pie,
                     ...
                     series: [{ val_ref: "Sheet1!$A$1:$A$2" }])
    xlsx_path = File.join(Dir.tmpdir, "dlbl_delete_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:dLbl><c:idx val="1"/><c:delete val="1"/></c:dLbl>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits dLbl with numFmt" do
    ...
    writer.add_chart(type: :pie,
                     ...
                     series: [{ val_ref: "Sheet1!$A$1:$A$2" }])
    xlsx_path = File.join(Dir.tmpdir, "dlbl_numfmt_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:dLbl><c:idx val="0"/><c:numFmt formatCode="0\.00%" sourceLinked="1"/>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits dLbl with spPr fill and line" do
    ...
    writer.add_chart(type: :pie,
                     ...
                     series: [{ val_ref: "Sheet1!$A$1:$A$2" }])
    xlsx_path = File.join(Dir.tmpdir, "dlbl_sppr_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:spPr><a:solidFill><a:srgbClr val="AABB00"/></a:solidFill><a:ln w="19050"><a:solidFill><a:srgbClr val="112233"/></a:solidFill></a:ln></c:spPr>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits dLbl with font" do
    ...
    writer.add_chart(type: :pie,
                     ...
                     series: [{ val_ref: "Sheet1!$A$1:$A$2" }])
    xlsx_path = File.join(Dir.tmpdir, "dlbl_font_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:txPr>.*<a:defRPr sz="1400" b="1">.*<a:solidFill><a:srgbClr val="FF0000"/></a:solidFill>}m, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
                     ...
                     series: [{ val_ref: "Sheet1!$A$1",
                                trendline: { type: "linear", disp_eq: true,
                                             label: { num_fmt: "0.00%",
                                                      ...
                                                      line_dash: "dash",
                                                      font: { size: 10, bold: true, color: "FF0000" } } } }])
    xlsx_path = File.join(Dir.tmpdir, "trendline_lbl_sppr_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(/<c:trendlineLbl>/, xml)
    assert_match(/<c:numFmt formatCode="0\.00%"/, xml)
    assert_match(%r{<c:spPr><a:solidFill><a:srgbClr val="FFFF00"/></a:solidFill><a:ln w="19050"><a:solidFill><a:srgbClr val="0000FF"/></a:solidFill><a:prstDash val="dash"/></a:ln></c:spPr>}, xml)
    assert_match(/<a:defRPr sz="1000" b="1">/, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  ...
  test "emits trendlineLbl with no_fill" do
    ...
    writer.add_chart(type: :line,
                     series: [{ val_ref: "Sheet1!$A$1",
                                trendline: { type: "linear", disp_eq: true,
                                             label: { no_fill: true } } }])
    xlsx_path = File.join(Dir.tmpdir, "trendline_lbl_nofill_#{Process.pid}.xlsx")
    writer.write(xlsx_path)
    xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:trendlineLbl><c:spPr><a:noFill/></c:spPr></c:trendlineLbl>}, xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "add_shape with blur emits a:effectLst with a:blur" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-blur", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    drawing_xml = read_xml_from_xlsx(xlsx_path, "xl/drawings/drawing1.xml")
    assert_match(%r{<a:effectLst><a:blur rad="50800" grow="0"/></a:effectLst>}, drawing_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "add_chart with series no_fill and no_line emits a:noFill" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-sernofill", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:spPr><a:noFill/><a:ln><a:noFill/></a:ln></c:spPr>}, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "add_chart with per-series smooth emits c:smooth inside c:ser" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-sersmooth", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:smooth val="1"/></c:ser>}, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "add_shape with text_font line_color emits a:ln inside a:rPr" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-textln", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    drawing_xml = read_xml_from_xlsx(xlsx_path, "xl/drawings/drawing1.xml")
    assert_match(%r{<a:rPr>.*<a:ln w="12700"><a:solidFill><a:srgbClr val="FF0000"/></a:solidFill></a:ln>.*</a:rPr>}m, drawing_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "add_shape with text_font u_fill_tx emits a:uFillTx in a:rPr" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-ufilltx", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    drawing_xml = read_xml_from_xlsx(xlsx_path, "xl/drawings/drawing1.xml")
    assert_match(%r{<a:uFillTx/>}, drawing_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "add_shape with text_font u_ln_tx emits a:uLnTx in a:rPr" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-ulntx", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    drawing_xml = read_xml_from_xlsx(xlsx_path, "xl/drawings/drawing1.xml")
    assert_match(%r{<a:uLnTx/>}, drawing_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "writes plot area line dash" do
    ...
    writer.set_cell("B1", 10)
    writer.add_chart(type: :bar, cat_ref: "Sheet1!A1", val_ref: "Sheet1!B1",
                     plot_area_line_color: "0000FF", plot_area_line_dash: "dash")

    xlsx_tempfile = Tempfile.new(["xlsxrb-pa-ld", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:plotArea>.*<c:spPr>.*<a:ln><a:solidFill><a:srgbClr val="0000FF"/></a:solidFill><a:prstDash val="dash"/></a:ln>.*</c:spPr>.*</c:plotArea>}m, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "writes plot area noFill" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-pa-nf", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:plotArea>.*<c:spPr><a:noFill/></c:spPr>.*</c:plotArea>}m, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits axis title noFill in spPr" do
    ...
    writer.add_chart(type: :bar,
                     ...
                     series: [{ val_ref: "Sheet1!$A$1" }])
    xlsx_tempfile = Tempfile.new(["xlsxrb-axtitlenf", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:catAx>.*<c:title>.*<c:spPr><a:noFill/></c:spPr></c:title>}m, chart_xml)
  ...
  test "emits chart title styling from flat parameters" do
    ...
    writer.add_chart(type: :bar,
                     ...
                     series: [{ val_ref: "Sheet1!$A$1" }])
    xlsx_tempfile = Tempfile.new(["xlsxrb-titleflat", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:title><c:tx><c:rich><a:bodyPr/><a:lstStyle/><a:p><a:r><a:rPr b="1" sz="1400"><a:solidFill><a:srgbClr val="FF0000"/></a:solidFill><a:latin typeface="Arial"/></a:rPr><a:t>Flat Styled</a:t></a:r></a:p></c:rich></c:tx>}m, chart_xml)
    assert_match(%r{<c:spPr><a:solidFill><a:srgbClr val="FFFF00"/></a:solidFill><a:ln w="12700"><a:solidFill><a:srgbClr val="000000"/></a:solidFill><a:prstDash val="dash"/></a:ln></c:spPr></c:title>}m, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "flat title_no_fill emits noFill in title spPr" do
    ...
    writer.add_chart(type: :bar,
                     ...
                     title_no_fill: true,
                     series: [{ val_ref: "Sheet1!$A$1" }])
    xlsx_tempfile = Tempfile.new(["xlsxrb-titlenf", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:title>.*<c:spPr><a:noFill/></c:spPr></c:title>}m, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits legend line_dash in spPr" do
    ...
    writer.add_chart(type: :bar,
                     ...
                     series: [{ val_ref: "Sheet1!$A$1" }])
    xlsx_tempfile = Tempfile.new(["xlsxrb-legdash", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:legend>.*<c:spPr><a:ln><a:solidFill><a:srgbClr val="000000"/></a:solidFill><a:prstDash val="dash"/></a:ln></c:spPr>}m, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits data table line_dash in spPr" do
    ...
    writer.add_chart(type: :bar,
                     ...
                     series: [{ val_ref: "Sheet1!$A$1" }])
    xlsx_tempfile = Tempfile.new(["xlsxrb-dtdash", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:dTable>.*<c:spPr><a:ln><a:solidFill><a:srgbClr val="999999"/></a:solidFill><a:prstDash val="dot"/></a:ln></c:spPr>}m, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits chart-level default font (chartSpace txPr)" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-chartfont", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{</c:chart><c:txPr><a:bodyPr/><a:lstStyle/><a:p><a:pPr><a:defRPr sz="1200" b="1"><a:solidFill><a:srgbClr val="333333"/></a:solidFill><a:latin typeface="Arial"/></a:defRPr></a:pPr><a:endParaRPr/></a:p></c:txPr>}, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
                     ...
                     series: [{ val_ref: "Sheet1!$A$1",
                                trendline: { type: "linear",
                                             label: { text: "Trend Label",
                                                      layout: { x: 0.5, y: 0.3 } } } }])
    xlsx_tempfile = Tempfile.new(["xlsxrb-tllayout", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:trendlineLbl><c:layout><c:manualLayout><c:x val="0.5"/><c:y val="0.3"/></c:manualLayout></c:layout><c:tx><c:rich><a:bodyPr/><a:lstStyle/><a:p><a:r><a:t>Trend Label</a:t></a:r></a:p></c:rich></c:tx>}, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits error bar fill and noFill in spPr" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-ebfill", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:spPr><a:solidFill><a:srgbClr val="FF0000"/></a:solidFill><a:ln><a:solidFill><a:srgbClr val="0000FF"/></a:solidFill></a:ln></c:spPr>}, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
  ...
  test "emits error bar noFill in spPr" do
    ...
    xlsx_tempfile = Tempfile.new(["xlsxrb-ebnofill", ".xlsx"])
    xlsx_path = xlsx_tempfile.path
    xlsx_tempfile.close
    writer.write(xlsx_path)

    chart_xml = read_xml_from_xlsx(xlsx_path, "xl/charts/chart1.xml")
    assert_match(%r{<c:spPr><a:noFill/></c:spPr>}, chart_xml)
  ensure
    File.delete(xlsx_path) if xlsx_path && File.exist?(xlsx_path)
  end
