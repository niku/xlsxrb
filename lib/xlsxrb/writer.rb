  ...
  class Writer
    ...
    def generate_chart_xml(chart)
      ...
      unless no_axes
        ...
        if chart[:cat_axis_line_color] || chart[:cat_axis_fill]
          parts << "<c:spPr>"
          parts << %(<a:solidFill>#{color_xml(chart[:cat_axis_fill])}</a:solidFill>) if chart[:cat_axis_fill]
          if chart[:cat_axis_line_color]
            w_attr = chart[:cat_axis_line_width] ? %( w="#{chart[:cat_axis_line_width]}") : ""
            parts << "<a:ln#{w_attr}><a:solidFill>#{srgb_clr_xml(chart[:cat_axis_line_color])}</a:solidFill></a:ln>"
          end
          parts << "</c:spPr>"
        end
        ...
        if chart[:val_axis_line_color] || chart[:val_axis_fill]
          parts << "<c:spPr>"
          parts << %(<a:solidFill>#{color_xml(chart[:val_axis_fill])}</a:solidFill>) if chart[:val_axis_fill]
          if chart[:val_axis_line_color]
            w_attr = chart[:val_axis_line_width] ? %( w="#{chart[:val_axis_line_width]}") : ""
            parts << "<a:ln#{w_attr}><a:solidFill>#{srgb_clr_xml(chart[:val_axis_line_color])}</a:solidFill></a:ln>"
          end
          parts << "</c:spPr>"
        end
      ...
      if chart[:plot_area_fill] || chart[:plot_area_line_color] || chart[:plot_area_line_dash]
        pa_sp = +""
      ...
      parts << "</c:chart>"
      if chart[:chart_fill] || chart[:chart_line_color]
        cs_sp = +""
        cs_sp << %(<a:solidFill>#{color_xml(chart[:chart_fill])}</a:solidFill>) if chart[:chart_fill]
        if chart[:chart_line_color]
          cs_lw = chart[:chart_line_width] ? %( w="#{(chart[:chart_line_width] * 12_700).to_i}") : ""
          cs_sp << "<a:ln#{cs_lw}><a:solidFill>#{color_xml(chart[:chart_line_color])}</a:solidFill></a:ln>"
        end
